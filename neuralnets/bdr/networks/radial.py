...
def build_nonstat_rbf(x_dim, landmarks, bw, reg_out, y_dim = 1,
    ...
    log_ell_X = tf.squeeze(tf.matmul(K_ell_X, params['ell_weights']) + params['ell_mu'])
    log_ell_ldmks = tf.squeeze(tf.matmul(K_ell_ldmks, params['ell_weights']) + params['ell_mu'])
